package org.nrg.xnatx.pipeline.helpers;
...
public class PipelineRepositoryHelper {

    public static final Logger logger = LoggerFactory.getLogger(PipelineRepositoryHelper.class);


    public PipelineRepositoryHelper(final PipePipelinerepository pipelineRepository ) {
            ...
            if (pipelines.size() > 0 ) {
                for (int i = 0; i < pipelines.size(); i++) {
                    ...
                    if (pipeline != null) {
                        ...
                        rowEntries[6] = "";
                        rowEntries[7] = pipelines.get(i).getStepid()==null ? pipelines.get(i).getName() : pipelines.get(i).getStepid();
            ...
            if (descendants.size() > 0 ) {
                for (int i = 0; i < descendants.size(); i++) {
                    ...
                    for (int j = 0; j < pipelinesDesc.size(); j++) {
                        ...
                        rowEntries[7] = pipelinesDesc.get(j).getStepid() == null ? pipelinesDesc.get(j).getName() : pipelinesDesc.get(j).getStepid();
                        _return.add(rowEntries);
                    }
                }
            }
    ...
    public PipePipelinedetails getPipeline(String path) {
        ...
        List<PipePipelinedetailsI> pipelines = pipelineRepository.getPipeline();
        for (int i = 0; i < pipelines.size(); i++) {
    ...
    public Hashtable<String, ArrayList<ArcPipelinedata>> getPipelinesForProject(XnatProjectdata project) throws Exception {
        ...
        return getPipelinesForDummyProject();
    }

    public Hashtable<String, ArrayList<ArcPipelinedata>> getPipelinesForDummyProject() throws Exception {
        Hashtable<String, ArrayList<ArcPipelinedata>> rtn = new Hashtable<String,ArrayList<ArcPipelinedata>>();
        Hashtable<String, ArrayList<PipePipelinedetails>> repository = getPipelinesPerDataType();
    ...
    public ArrayList<ArcProjectPipeline> getArcPipelinesForProject(ArrayList<PipePipelinedetails> pipelines) throws Exception {
        ...
        for (int i = 0; i < pipelines.size(); i++) {
            ...
            pBean.setStepid(pipelineDescriptor.getName());
            //}
    ...
    public String getDisplayName(final String dataType) {
        try {
            final ElementSecurity es = ElementSecurity.GetElementSecurities().get(dataType);
            if (es != null) {
    ...
    public ArcProject createNewArcProject(XnatProjectdata proj) throws Exception {
        ...
        return createNewArcProjectForDummyProject();
    }


    public ArcProject createNewArcProjectForDummyProject() throws Exception {
        ArcProject arcProject = new ArcProject();
        Hashtable<String, ArrayList<ArcPipelinedata>>  pipelinesHash = PipelineRepositoryManager.GetInstance().getPipelinesForDummyProject();
                ...
                if (dataType.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) {
                    ...
                    if (projectPipelines != null && projectPipelines.size() > 0) {
                        for (int i = 0; i < projectPipelines.size(); i++) {
                            ArcPipelinedata p = projectPipelines.get(i);
                            p.setName(getName(p.getName()));
                            arcProject.setPipelines_pipeline(p);
                        }
                    }
                }else {
    ...
    public ArcProject createNewArcProjectForDummyProject(String xsiType) throws Exception {
        ArcProject arcProject = new ArcProject();
        Hashtable<String, ArrayList<ArcPipelinedata>>  pipelinesHash = PipelineRepositoryManager.GetInstance().getPipelinesForDummyProject();
        if (pipelinesHash != null && pipelinesHash.size() > 0) {
            Enumeration<String> keys = pipelinesHash.keys();
            while (keys.hasMoreElements()) {
                ...
                if (xsiType.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) {
                    ...
                    if (dataType.equals(xsiType) || dataType.equals("All Datatypes")){
                        ArrayList<ArcPipelinedata> projectPipelines = pipelinesHash.get(dataType);
                        if (projectPipelines != null && projectPipelines.size() > 0) {
                            ArcProjectDescendant arcProjectDesc = new ArcProjectDescendant();
                            arcProjectDesc.setXsitype(dataType);
    ...
    public void setPipeline(final PipePipelinedetails pipelineDetails) throws Exception {
        pipelineRepository.setPipeline(pipelineDetails);
    }

    public PipePipelinerepository getPipelineRepository() {
        return pipelineRepository;
    }


    private final PipePipelinerepository pipelineRepository;
