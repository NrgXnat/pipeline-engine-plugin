/*
 * web: org.nrg.xnat.turbine.modules.actions.ManagePipeline
...
public class ManagePipeline extends SecureAction {
    ...
    public void doAdd(RunData data, Context context) throws Exception {
		...
		try {
            ...
            ValidationResultsI temp = pipelineDetails.getItem().validate();
    ...
    public void doLaunchpipeline(RunData data, Context context) throws Exception {
        ...
        pipelineLaunchParameters.setPipelineName(pipeline_path);
            String exptLabel = item.getStringProperty("label");
            String project = item.getStringProperty("project");
        pipelineLaunchParameters.setId(item.getStringProperty("ID"));
        pipelineLaunchParameters.setLabel(exptLabel);
        pipelineLaunchParameters.setExternalId(project);
        ...
        pipelineLaunchParameters.setWaitFor(wait_for);

            boolean runPipelineInProcess = data.getParameters().containsKey("run_pipeline_in_process") ? data.getParameters().getBoolean("run_pipeline_in_process") : PipelineConstants.DEFAULT_RUN_PIPELINE_IN_PROCESS;
        pipelineLaunchParameters.setRunPipelineInProcess(runPipelineInProcess);
            ...
            // isn't selected.
            boolean recordWorkflowEntries = data.getParameters().containsKey("suppress_workflow_entries") ? !data.getParameters().getBoolean("suppress_workflow_entries") : PipelineConstants.DEFAULT_RECORD_WORKFLOW_ENTRIES;
        pipelineLaunchParameters.setRecordWorkflowEntries(recordWorkflowEntries);
